...
# SET PARAMETERS
params = Parameters()
params.nbNoeuds = params.nbNoeuds + 1
...
ffcn_impact = casadi.Function("ffcn_impact",
                               [x, u, p],
...
for k in range(params.nbNoeuds_swing):
    ...
    Je += Fcn_Objective.fcn_objective_emg(params.wU, Uk[:params.nbMus], U_real_swing[:, k])
    Ja += Fcn_Objective.fcn_objective_activation(params.wL, Uk[:params.nbMus])
    Jt += Fcn_Objective.fcn_objective_residualtorque(params.wt, Uk[params.nbMus:])

# ------------ PHASE 3 : Impact
Uk = U[(params.nbU * params.nbNoeuds_stance) + (params.nbU * params.nbNoeuds_swing): (params.nbU * params.nbNoeuds_stance) + (params.nbU * params.nbNoeuds_swing) + params.nbU]
Xk = X[(params.nbX * params.nbNoeuds_stance) + (params.nbX * params.nbNoeuds_swing): (params.nbX * params.nbNoeuds_stance) + (params.nbX * params.nbNoeuds_swing) + params.nbX]
G.append(X[-params.nbX:] - int_RK4(ffcn_no_contact, params, Xk, Uk, P))

# OBJECTIVE FUNCTION
M = markers(Xk)
Jm += Fcn_Objective.fcn_objective_markers_casadi(params.model_stance, params.wMa, params.wMt, M, M_real_swing[:, :, k])
Je += Fcn_Objective.fcn_objective_emg(params.wU, Uk, U_real_swing[:, k])
Ja += Fcn_Objective.fcn_objective_activation(params.wL, Uk)
Jt += Fcn_Objective.fcn_objective_residualtorque(params.wt, Uk[params.nbMus:])

# ----------------------------- Contraintes ----------------------------------------------------------------------------
G.append(X[:params.nbX] - X[-params.nbX:])    # cyclic
lbg = [0] * params.nbX * params.nbNoeuds + [0] * params.nbX
ubg = [0] * params.nbX * params.nbNoeuds + [0] * params.nbX
...
u0[: params.nbMus, :] = load_initialguess_muscularExcitation(np.hstack([U_real_stance[:, :-1], U_real_swing[:, :]]))
# u0[: params.nbMus, :]   = np.zeros((params.nbMus, params.nbNoeuds)) + init_A
u0[params.nbMus + 1, :] = [-500] * params.nbNoeuds_stance + [0] * params.nbNoeuds_swing + [0]

# STATE
X0                 = np.zeros((params.nbX, (params.nbNoeuds + 1)))
X0[:params.nbQ, :-1] = load_initialguess_q(params, 'cycle')
...
opts   = {"ipopt.tol": 1e-2, "ipopt.linear_solver": "ma57", "ipopt.hessian_approximation":"limited-memory","ipopt.acceptable_obj_change_tol": 1e2, "ipopt.max_iter": 10000}
...
# save txt file
file = '/home/leasanchez/programmation/Simu_Marche_Casadi/Resultats/equincocont01/RES/equincocont01_gait_3.txt'
f = open(file, 'a')
f.write('STATE\n\n')
