...
def affichage_markers_result(sol_q, T_phase, nbNoeuds_phase, nbMarker, M_real):
    ...
    for k_swing in range(nbNoeuds_phase[1]):
        model = biorbd.Model('/home/leasanchez/programmation/Marche_Florent/ModelesS2M/ANsWER_Rleg_6dof_17muscle_0contact.bioMod')
        markers = model.markers(sol_q[:, nbNoeuds_phase[0] + 1 + k_swing])
        for nMark in range(nbMarker):
            M_simu[:, nMark, nbNoeuds_phase[0] + 1 + k_swing] = markers[nMark].to_array()
    ...
    for k in range(nbNoeuds):
        ...
        plt.plot([M_real[0, 2, k], M_real[0, 4, k], M_real[0, 11, k], M_real[0, 19, k], M_real[0, 22, k]], [M_real[2, 2, k], M_real[2, 4, k], M_real[2, 11, k], M_real[2, 19, k], M_real[2, 22, k]], 'r+')
    ...
    diff = M_simu[:, :, :-1] - M_real             # 3 x nMarker x nbNoeuds
