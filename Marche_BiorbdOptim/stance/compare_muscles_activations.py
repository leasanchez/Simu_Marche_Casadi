...
for i in range(biorbd_model.nbMuscleTotal()):
    ...
    rms_mus_state[i] = np.sqrt(
        np.mean((mus_state[i, :] - activation_ref[i, :]) * (mus_state[i, :] - activation_ref[i, :]))
    )
    rms_mus_shift[i] = np.sqrt(
        np.mean((mus_shift[i, :] - activation_ref[i, :]) * (mus_shift[i, :] - activation_ref[i, :]))
    )
    rms_mus_shift_ref[i] = np.sqrt(
        np.mean((mus_shift_ref[i, :] - activation_ref[i, :]) * (mus_shift_ref[i, :] - activation_ref[i, :]))
    )
