...
def prepare_ocp(
    biorbd_model, final_time, nb_shooting, markers_ref, activation_ref, q_ref,
):
    # Problem parameters
...
if __name__ == "__main__":
    ...
    # Track these data
    biorbd_model = biorbd.Model("../../ModelesS2M/ANsWER_Rleg_6dof_17muscle_0contact.bioMod")
    ocp = prepare_ocp(biorbd_model, final_time, n_shooting_points, markers_ref, activation_ref, q_ref,)
    ...
    markers_func_all = Function(
        "ForwardKin_all",
        [symbolic_states],
        [biorbd_model.markers(symbolic_states[: biorbd_model.nbQ()])],
        ["q"],
        ["markers"],
    ).expand()
    ...
    ocp.add_plot(
        ...
        update_function=lambda x, u: markers_ref[0, :, :],
    ...
    ocp.add_plot(
        "Markers plot coordinates",
        update_function=lambda x, u: get_markers_pos(x, 0),
    ...
    ocp.add_plot(
        "Markers plot coordinates",
        update_function=lambda x, u: get_markers_pos(x, 1),
        plot_type=PlotType.PLOT,
        color="tab:green",
    )
    ocp.add_plot(
    ...
    ocp.add_plot(
        "Markers plot coordinates",
        update_function=lambda x, u: get_markers_pos(x, 2),
        plot_type=PlotType.PLOT,
        color="tab:blue",
    )
    ...
    for i in range(2):
        axes[i].bar(
            np.linspace(0, nb_markers, nb_markers),
            hist_diff_track[2 * i, :],
            width=1.0,
            facecolor="b",
            edgecolor="k",
