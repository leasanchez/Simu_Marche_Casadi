...
nbU      = nbMus + 3                                   # number of controls : muscle excitation + pelvis forces
nbX      = 2*nbQ + nbMus                               # number of states : generalized positions + velocities + muscle activation
...
# # VISUALIZATION INPUT
# affichage_emg_real(U_real, T_stance, T_swing, nbNoeuds_stance, nbNoeuds_swing)
# affichage_markers_real(M_real)
# affichage_GRF_real(GRF_real, T_stance, T_swing, nbNoeuds_stance, nbNoeuds_swing)
...
# VISUALIZATION OF CONVERGENCE FOR EACH
...
min_Q = -50
max_Q = 50
lbX   = ([min_Q]*2*nbQ + [min_A]*nbMus)*(nbNoeuds + 1)
ubX   = ([max_Q]*2*nbQ + [max_A]*nbMus)*(nbNoeuds + 1)
...
X0[:nbQ, :]       = q0
X0[nbQ: 2*nbQ, :] = dq0
X0[2*nbQ:, :]     = np.zeros((nbMus, (nbNoeuds + 1))) + init_A
X0                = vertcat(*X0.T)
...
for k in range(nbNoeuds):
    ...
    sol_F[:, k]  = np.reshape(uk[nbMus:], (3*1))
